'use strict';
// templates/value-driver-tree.js — 价值驱动树（财务分解：左节点 → 中间算子 → 右子节点）
...
module.exports = {
  ...
  version:     '1.0.0',
  category:    '咨询框架',
  description: '左侧顶层指标，右侧逐层分解到 2-3 层驱动因素，每节点含数值与算子',
  ...
  usage: {
    when:    '财务指标拆解到驱动因素：营收 = 客户数 × 客单价 = ...',
    notWhen: '非可拆解指标；纯并列概念用 threeColumn',
    maxItems: 4,
    typicalHeight: '3.5"',
  },
  ...
  fromKeyPoints(keyPoints, page) {
    ...
    const kps = keyPoints || [];
    const drivers = kps.slice(0, 4).map((kp, i) => {
  ...
  render(pres, slide, data, infra) {
    ...
    drivers.slice(0, n).forEach((d, i) => {
      ...
      // 算子（在 root 和 driver 之间）
      const opChar = d.op || (i === 0 ? '=' : '×');
